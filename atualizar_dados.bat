...
echo [%date% %time%] [6/7] DDR por Lançamento... >> logs\atualizacao.log
...
echo [%date% %time%] [7/7] RPP por Nota de Empenho... >> logs\atualizacao.log
python extrair_rpp_por_ne.py >> logs\atualizacao.log 2>&1
if %errorlevel% neq 0 (
    echo [%date% %time%] ERRO no painel RPP por NE. >> logs\atualizacao.log
) else (
    echo [%date% %time%] OK - RPP por NE concluído. >> logs\atualizacao.log
)
